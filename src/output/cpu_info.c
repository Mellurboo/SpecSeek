...
void print_cpu_cpuid_supported_leafs(cpu_t cpu_ctx, int min_verbosity){
    IF_VERBOSE(min_verbosity){
        ...
        printf("%sMax Supported CPUID Standard Leaf: %s0x%X\n", WHITE, VENDOR_COLOUR, cpu_get_max_supported_leaf());
        printf("%sMax Supported CPUID Extended Leaf: %s0x%X", WHITE, VENDOR_COLOUR, cpu_get_max_supported_extended_leaf());
...
void print_cpu_processor_identifiers(cpu_t cpu_ctx){
    ...
    printf("%sName:      %-8s%s\n", WHITE, VENDOR_COLOUR, cpu_ctx.name);
    ...
    printf("%sVendor:    %-8s%s\n", WHITE, VENDOR_COLOUR, cpu_ctx.vendor);
    TERM_DIVIDER_SMALL("identifiers", BOLD_VENDOR_COLOUR);
    printf("%sModel: %-10s0x%X\n", WHITE, VENDOR_COLOUR, cpu_ctx.model);
    printf(" %s%sBase Model:%-9s0x%X\n", SYMBOL_LIST_CHILD(VENDOR_COLOUR), WHITE, VENDOR_COLOUR, cpu_ctx.base_model);
    printf(" %s%sExt. Model:%-9s0x%X\n", SYMBOL_LIST_END(VENDOR_COLOUR), WHITE, VENDOR_COLOUR, cpu_ctx.ext_model);
    printf("%sFamily: %-9s0x%X\n", WHITE, VENDOR_COLOUR, cpu_ctx.family);
    printf(" %s%sBase Family:%-8s0x%X\n", SYMBOL_LIST_CHILD(VENDOR_COLOUR), WHITE, VENDOR_COLOUR, cpu_ctx.base_family);
    printf(" %s%sExt. Family:%-8s0x%X\n", SYMBOL_LIST_END(VENDOR_COLOUR), WHITE, VENDOR_COLOUR, cpu_ctx.ext_family);
    printf("%sRevision:%-8s0x%X", WHITE, VENDOR_COLOUR, cpu_ctx.revision);
}
...
void print_cpu_specification(cpu_t cpu_ctx){
    ...
    printf("%sLogical Processors: %-8s%u\n", WHITE, VENDOR_COLOUR, cpu_ctx.logical_processors);
    printf(" %s%sPhysical Cores:  %-9s%u\n", SYMBOL_LIST_CHILD(VENDOR_COLOUR), WHITE, VENDOR_COLOUR, cpu_ctx.physical_processors);
    printf(" %s%sThreads Per Core: %-8s%u\n", SYMBOL_LIST_END(VENDOR_COLOUR), WHITE, VENDOR_COLOUR, cpu_ctx.thread_count_per_core);
    
    IF_VENDOR_INTEL({
        if (cpu_supports_standard_leaf(0x0000001A)){
            printf("  %sPerformance Cores: %s%u\n", WHITE, VENDOR_COLOUR, cpu_ctx.performance_cores);
            printf("  %sEfficiency Cores: %s%u\n", WHITE, VENDOR_COLOUR, cpu_ctx.efficient_cores);
        }
    });
}
...
void print_cpu_features(cpu_t cpu_ctx){
    TERM_DIVIDER("CPUs Feature Support", YELLOW);
    TERM_DIVIDER_SMALL("Standard Features", YELLOW);
    reset_cpu_feature_count();
    cpu_check_standard_features();

    TERM_DIVIDER_SMALL("Extended Features", YELLOW);
    reset_cpu_feature_count();
    cpu_check_extended_features();

    TERM_DIVIDER_SMALL(" Extra. Features ", YELLOW);
    reset_cpu_feature_count();
    cpu_check_misc_features();
    TERM_DIVIDER_NOTEXT(YELLOW);
}
...
void print_cpu_clock_info(){
    ...
    printf("%sCrystal Clock Frequency = %s%uhz\n", WHITE, WHITE, cpu_get_nominal_frequency());
}
